 ...
 *
 * The rule itself lives in the kit (`require-named-lifetime`) rather than as
 * a local `no-restricted-syntax` entry, and that is not incidental:
 * `no-restricted-syntax` options do not merge, so a workspace-level entry
 * silently replaces `extensions-security.config.ts`'s own selectors — the
 * exact hazard that file's header warns about. A named rule cannot be
 * clobbered that way.
 */
const namedLifetime = {
  files: ["src/**/*.{ts,tsx}"],
