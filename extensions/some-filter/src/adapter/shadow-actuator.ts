...
function staticShadowLayer(): CSSStyleSheet {
  ...
  for (const rule of DARK_THEME_BODY_RULES) {
    try {
      ...
      // surface left unthemed, with the failure visible nowhere. The array
      // is a growing set of rules maintained by hand, and this extension
      // ships to two engines whose selector support is not identical, so
      // "no rule in it is ever unparseable anywhere" is not an assumption
      // worth resting a whole layer on.
      //
