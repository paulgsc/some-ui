...
| "Kept awake" staleness    | `discard-state.ts`'s badge/banner entry for a tab is only refreshed on the _next_ discard attempt (success clears it, another refusal re-stamps it) or when the tab closes. If the user resolves the cause manually (submits the form, stops the audio) without the periodic sweep revisiting that tab, the badge can overstate reality for up to one check interval (~3–20 min). No push signal exists for "the veto condition just cleared." | Cosmetic — self-heals on the next sweep or tab close                       |
