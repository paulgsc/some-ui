...
/**
 * A single ambient callout, not a per-tab audit. Chromium's own discard veto
 * exposes no reason string to extensions, so a per-tab breakdown would either
...
export function DiscardBanner({ reasons }: DiscardBannerProps): HTMLElement {
  ...
  heading.textContent = "Some tabs were kept awake by Chrome"
