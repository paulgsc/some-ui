 ...
 * the tab live — Chromium's own `CanDiscardTab` safety check (unsaved form
 ...
 * either a no-op success or an outright refusal. Form input is the common
 * case in practice, but `beforeunload` and DevTools land in the same bucket —
 * the API gives no way to tell them apart, and guessing a specific cause we
 * cannot verify would be its own kind of misleading.
 */
...
const REASON_LABEL: Record<DiscardSkipReason, string> = {
  protected: "Unsaved text or form inputs",
  media: "Active audio or video",
}
...
export async function hydrateDiscardState(): Promise<void> {
  ...
  if (raw !== null && typeof raw === "object") {
    for (const [key, entry] of Object.entries(raw)) {
      ...
      if (Number.isFinite(tabId) && isSkipEntry(entry)) {
