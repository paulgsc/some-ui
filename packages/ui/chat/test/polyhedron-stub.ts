 ...
 * `await import("polyhedron")`. Vite's import-analysis resolves that
 * specifier eagerly while transforming the module graph - even though
 * nothing in these tests ever calls `getWasmManager()` to actually load it -
 * so without a real (wasm-pack-built) `polyhedron` package on disk, merely
 * importing anything from `some-ui-utils`'s barrel fails at transform time.
