...
import { CheckCircle2, EyeOff, RotateCcw, Trophy, XCircle, Zap } from "lucide-react"
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-card-foreground">Session Complete</h2>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-sm text-muted-foreground">{challenge.title}</span>
              <span className={cn("text-xs font-medium capitalize", DIFFICULTY_COLORS[challenge.difficulty])}>
        ...
        <div className="flex flex-col items-center gap-1 rounded-lg border border-primary/30 bg-primary/10 px-4 py-2">
          ...
          <span className="text-[10px] text-muted-foreground uppercase tracking-wide">XP earned</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCell label="Time" value={formatTime(Math.round(solve.elapsedTime))} />
      ...
      <div className="rounded-lg border bg-muted/30 p-3 text-xs text-muted-foreground space-y-1">
        <p className="font-medium text-card-foreground text-sm mb-2">XP Breakdown</p>
        <div className="flex justify-between">
          <span>Base ({challenge.difficulty})</span>
          <span className="font-mono">
            {challenge.difficulty === "easy" ? 10 : challenge.difficulty === "medium" ? 25 : 50}
          </span>
        </div>
        {solve.wpm >= 40 && (
          <div className="flex justify-between text-primary">
            <span>Speed bonus (≥40 WPM)</span>
            <span className="font-mono">×{solve.wpm >= 80 ? "2.0" : solve.wpm >= 60 ? "1.5" : "1.2"}</span>
          </div>
        )}
        {solve.accuracy < 95 && (
          <div className="flex justify-between text-destructive">
            <span>Accuracy penalty (&lt;95%)</span>
            <span className="font-mono">×{solve.accuracy >= 80 ? "0.8" : "0.6"}</span>
          </div>
        )}
        {wasHidden && (
          <div className="flex justify-between text-primary">
        ...
        {solve.n && (
          <div className="flex justify-between">
            ...
            <span className="font-mono">
              ×{solve.n === "tiny" ? "1.0" : solve.n === "small" ? "1.5" : solve.n === "medium" ? "2.0" : "3.0"}
            </span>
          </div>
        )}
