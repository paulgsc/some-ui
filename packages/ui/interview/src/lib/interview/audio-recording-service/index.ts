  ...
  async requestPermission(): Promise<MediaStream> {
    ...
    try {
      ...
      if (error instanceof DOMException) {
        // eslint-disable-next-line switch-lint/require-fail-fast-default -- error.name is an open string set; unhandled names fall through to the generic throw below, which is the fail-fast catch-all
        switch (error.name) {
