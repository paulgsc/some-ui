...
const Cell: React.FC<CellProps> = ({ label, value, color = "neutral" }) => {
  ...
  return (
    <div className="flex flex-col gap-0.5 border-r border-neutral-800 px-4 py-3 last:border-r-0">
      ...
      <span className={`font-mono text-[14px] font-semibold ${colorClass}`}>
        {value}
      </span>
    </div>
  )
...
export const MetricsBar: React.FC<MetricsBarProps> = ({ metrics, greeks }) => (
  <div className="flex flex-col">
    <div className="flex border-t border-neutral-800">
      <Cell
        label="P/L at spot"
        value={fmtDollar(metrics.plAtSpot)}
        color={metrics.plAtSpot >= 0 ? "green" : "red"}
      />
      <Cell
        label="max profit"
        value={fmtDollar(metrics.maxProfit)}
        color="green"
      />
    ...
    <div className="flex border-t border-neutral-800 bg-neutral-900/40">
      <Cell
        ...
        color={
          greeks.delta > 0 ? "green" : greeks.delta < 0 ? "red" : "neutral"
        }
      />
      <Cell label="Γ gamma" value={greeks.gamma.toFixed(4)} />
      <Cell
        label="Θ theta/d"
        value={fmtDollar(greeks.theta * 100)}
        color={greeks.theta > 0 ? "green" : "red"}
      />
      <Cell
        label="ν vega/1%"
        value={fmtDollar(greeks.vega * 100)}
        color={greeks.vega > 0 ? "green" : "red"}
      />
      <Cell
        label="θ/ν ratio"
        value={
          greeks.vega !== 0 ? (greeks.theta / greeks.vega).toFixed(2) : "—"
        }
      />
    </div>
  </div>
