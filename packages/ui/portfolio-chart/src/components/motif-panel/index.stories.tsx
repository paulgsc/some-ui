...
function uuid() { return Math.random().toString(36).slice(2, 8) }

const SEED_POINTS: Array<MotifPoint> = [
  { id: "1", text: "short premium strangle around weekly NVDA ATR", checked: true },
...
const Controlled: React.FC<{ initial: Array<MotifPoint> }> = ({ initial }) => {
  ...
  const toggle = (id: string) =>
    setPoints((ps) => ps.map((p) => (p.id === id ? { ...p, checked: !p.checked } : p)))
  ...
  return <MotifPanel points={points} onToggle={toggle} onAdd={add} onRemove={remove} onUpdate={update} />
}
...
export const AllChecked: Story = {
  render: () => <Controlled initial={SEED_POINTS.map((p) => ({ ...p, checked: true }))} />,
}
