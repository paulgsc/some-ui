...
export const resumeData: Record<ResumeVariant, ResumeData> = {
  ...
  "systems": {
    ...
    "projects": [
      {
        "name": "Real-time transport and work pipeline",
        "kind": "Rust, WebSockets, NATS JetStream, Tokio",
        "premise": "A real-time system must make admission, liveness, redelivery, and termination visible at every process boundary.",
        "bullets": [
      ...
      {
        "name": "Study intervention engine",
        "kind": "Pure Rust domain crates, SQLite, Web Push",
        "premise": "Time may constrain an intervention, but the clock should not manufacture its reason.",
        "bullets": [
      ...
      },
      {
        ...
        "kind": "Firefox MV3, TypeScript, WebAssembly, Playwright",
        "premise": "Restart is normal for an event-driven worker, and browser state must survive it without hidden runtime coupling.",
        "bullets": [
    ...
    "toolbox": [
      ...
      {
        "label": "Concurrency",
        "items": "Tokio, actors, permits, cancellation, backpressure"
      },
      {
        "label": "Messaging",
        "items": "NATS JetStream, WebSockets, typed jobs"
      },
      {
        "label": "Data",
        "items": "SQLx, SQLite, Redis, coalesced caching"
      },
      {
        "label": "Operations",
        "items": "Docker, distroless, Prometheus, Grafana, Jsonnet"
      },
      {
        "label": "Quality",
        "items": "Clippy, cargo-deny, route parity, contract, browser"
      }
    ],
    "repositories": [
      {
  ...
  "learning": {
    ...
    "projects": [
      ...
      {
        "name": "Production study-session backend",
        "kind": "file_host, Rust, Axum, SQLx, Redis",
        "premise": "Local progress and the server's durable view need an explicit contract, migration path, and failure policy.",
        "bullets": [
      ...
      {
        "name": "Event-driven study nudge",
        "kind": "Rust policy crates, Web Push, asynchronous worker",
        "premise": "A reminder is useful only when declining engagement warrants it and consent, presence, cooldown, and local time permit it.",
        "bullets": [
    ...
    "toolbox": [
      ...
      {
        "label": "Services",
        "items": "Axum, SQLx, Redis, Web Push"
      },
      {
        "label": "Modeling",
